...
public class UserTest {
	...
	public void testUpdateUser() {
		userPayload.setJob(faker.job().title());
		Response response=UsersEndPoints.updateUser(user_id, userPayload);
		response.then()
			.statusCode(200)
			.header("Content-Type", containsString("application/json"))
			.header("Server", notNullValue())
	...
	public void testGetUser() {
		...
		Response response=UsersEndPoints.getUser(userPayload.getUser_id());
		response.then()
		.statusCode(200)
		.log().all();
	...
	public void testDeleteUser() {
		...
		Response response=UsersEndPoints.deleteUser(userPayload.getUser_id());
		response.then()
		.statusCode(204);
